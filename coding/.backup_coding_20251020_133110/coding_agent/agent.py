...
code_agent = LlmAgent(
    ...
    name="coding_agent",
    instruction="""
    You are a coding agent, create a python script and only python script. Do not answer the question, do not provide any opinion on the question. 
    If it needs some parameters create command line parameters for the script. IMPORTANT: Only generate the code. Do not attempt to run, execute, or test the code. Just provide the code and stop.
    """,
...
root_agent = LlmAgent(
    ...
    name="root_agent",
    instruction="""
    You are a helpfull research assistant, your task is to manage process of validating research ideas.
    The best way to validate idea is to create code to test the idea, run the code and interpret the results.
    Idea cannot be validated vithout sufficient volume of calculations.
    """,
